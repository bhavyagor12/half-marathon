...
const directory = new URL('../output/avatar/', import.meta.url);
...
if (command === 'submit') {
  ...
  const input = await readFile(new URL('bhavya-modeling-reference.png', directory));
  const parameters = {
    ai_model: 'meshy-7', model_type: 'standard', ultra_mode: true,
  ...
  saved = {status: 'SUBMITTING', inputSha256: createHash('sha256').update(input).digest('hex'), parameters};
  await writeFile(stateFile, JSON.stringify(saved, null, 2) + '\n', {flag: 'wx'});
  const response = await api(endpoint, {...parameters, image_url: `data:image/png;base64,${input.toString('base64')}`});
