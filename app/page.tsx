...
export default function Home() {
  ...
  const openCount = 9 - sponsors.length, panelVisible = !!panel && panelShown;
  ...
  return <main className={`experience ${panelVisible ? 'has-panel' : ''} ${capture ? 'capture' : ''}`}>
    ...
    <div className="main-action hud">
      <button className="primary t-learn" onClick={() => open('spots')}>{error ? 'Explore sponsorship spots' : !loaded ? 'View spots · from $10 USD' : openCount ? `View ${openCount} open spots · from $10 USD` : 'View sponsors · take over a spot'}<Chevron/></button>
    ...
    {panel && <dialog open ref={dialog} data-open={panelShown} className={`panel t-panel-slide ${panel === 'spots' || panel === 'detail' ? 'spots-panel' : ''} ${panel === 'detail' ? 'detail-panel' : ''}`} aria-labelledby="panel-title" aria-modal="false">
      ...
      <div className="panel-content"><div key={`${panel}-${selected}`} className="panel-swap">
        {panel === 'spots' && <>
          <p className="panel-intro">Your logo on my race kit: chest, back, forearms and quad from $10 USD; the premium butt spot from $20. Each takeover doubles the price.</p>
        ...
        {panel === 'story' && <>
          ...
          <button className="primary t-learn" onClick={() => open('spots')}>Find your spot · from $10 USD<Chevron/></button>
        ...
        {panel === 'rules' && <div id="rules">
          <h2>The auction</h2><p>Eight spots on my tee, forearms and quad start at $10 USD. The premium butt spot on the back of the shorts starts at $20 USD. Each takeover doubles that spot’s price. Any checkout taxes are shown before payment. The sponsor holding the spot when bidding closes gets the race-day placement: printed on the kit, or a temporary tattoo on skin spots.</p>
