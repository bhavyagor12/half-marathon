...
type RefundJob={payment:string;order_id:string;mode:string;status:string;amount:number|null;fee:number|null;refund_id:string|null;rail:string|null};
// Jobs in these states are finished, with Dodo, or waiting on a person; the automatic queue leaves them alone.
const PARKED=['succeeded','submitted','awaiting_wallet','awaiting_payout'];
export async function processRefund(paymentId:string){const {data:job,error:loadError}=await db().from('slowrun_refund_jobs').select('*').eq('payment',paymentId).maybeSingle<RefundJob>();if(loadError)throw loadError;if(!job||PARKED.includes(job.status))return;if(job.status==='failed')throw new Error('Refund requires operator review');const client=dodo();
